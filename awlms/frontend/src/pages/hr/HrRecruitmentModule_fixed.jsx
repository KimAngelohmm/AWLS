    ...
    <div className="rec-modal-overlay" onClick={onClose}>
      <div className="rec-modal" onClick={(e) => e.stopPropagation()}>
        <div className="rec-modal-head">
          ...
          <button type="button" className="rec-modal-close" onClick={onClose} aria-label="Close">X</button>
        </div>
          ...
          <div>
            ...
            {data && (
              <p className="rec-modal-sub">
                {data.full_name} - {data.job_title} - {data.interview_status}
              </p>
            )}
          </div>
          <button type="button" className="rec-modal-close" onClick={onClose} aria-label="Close">X</button>
              ...
              <div className="rec-modal-section">
                ...
                {data.ai_recommendation && (
                  <p className="rec-modal-rec">
                    AI Recommendation: {data.ai_recommendation === 'hire' ? 'Hire' : 'No Hire'}
                  </p>
                )}
              </div>
            )}
...
export default function HrRecruitmentModule() {
  ...
  async function sendInterviewLink(applicantId, email, subject, jobId) {
    setSaving(true);
    setLoadError('');
    try {
      ...
      setLoadError(err.body?.error || err.message || 'Failed to send link');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="hr-page">
      ...
      {dashError && <div className="auth-alert" role="alert">{dashError}</div>}
      {loadError && <div className="auth-alert" role="alert">{loadError}</div>}
        ...
        <div className="hr-table-wrap">
          <table className="hr-table">
            ...
            <tbody>
              {jobs.length === 0 ? (
                <tr>
                  <td colSpan={7} className="hr-table-empty">No positions yet. Create one above.</td>
                </tr>
              ) : (
                jobs.flatMap((j) => {
                                        ...
                                        <td className="rec-actions">
                                          {a.hiring_decision === 'pending_review' ? (
                                            <button
                                              ...
                                              disabled={saving}
                                              onClick={() => setSendLinkApplicant(a)}
                                            >
      ...
      <section className="hr-panel">
        ...
        <p className="hr-panel-sub muted">
          Includes AI-completed interviews (interview_status = completed) and legacy rows with a summary.
          ...
          <table className="hr-table">
            ...
            <tbody>
              {assessments.length === 0 ? (
                <tr>
                  <td colSpan={7} className="hr-table-empty">No assessments in review.</td>
                </tr>
              ) : (
                assessments.map((a) => (
                  <tr key={a.id}>
      ...
      {viewTranscriptId && (
        <TranscriptModal applicantId={viewTranscriptId} onClose={() => setViewTranscriptId(null)} />
      )}

      {sendLinkApplicant && (
        <SendInterviewLinkModal
          applicant={sendLinkApplicant}
          interviewLink={`${origin}/interview/${sendLinkApplicant.id}`}
          onClose={() => setSendLinkApplicant(null)}
          onSend={(email, subject) => sendInterviewLink(sendLinkApplicant.id, email, subject, sendLinkApplicant.jobId)}
        />
      )}
    </div>
  );
